...
public static class ChaosTokenEvents
{
    ...
    public static void HandleVotesEventHandler(HandleVoteEvent @event)
    {
        ...
        if (@event.VoteData.Owner.HasModifier<TokenVotes>())
        {
            ...
            while (chance > 0)
            {
                if (UnityEngine.Random.RandomRange(1, 100) < chance)
                {
                    votes++;
                    chance -= UnityEngine.Random.RandomRange(15, 35);
                }
                else
                {
                    break;
    ...
    {
        if (OptionGroupSingleton<ChaosTokensModifierOptions>.Instance.ChaosTokenAmount < 1)
        {
            return;
        }
        ...
        if (@event.TriggeredByIntro)
        {
            ...
            if (!OptionGroupSingleton<ChaosTokensOptions>.Instance.TokensEnabledFirstRound)
            {
                return;
            }
        }
        ...
        int tokensToHandle = Mathf.Clamp(UnityEngine.Random.RandomRangeInt(min, max), 0, potentialPlayers.Count);
        while (tokensToHandle > 0 && potentialPlayers.Count > 0)
        {
            var player = potentialPlayers[0];
            // Reset when player gets a token
            PriorityTable[player.PlayerId] = 10;
            
            int tokens = 1;
            if (UnityEngine.Random.RandomRangeInt(1, 100) < DoubleTokenChance && tokensToHandle > 1)
            {
                tokens = 2;
                PriorityTable[player.PlayerId] = 11; // double token decreases the priority
            }

            player.RpcChaosTokenIncrease(tokens, true);
            
            tokensToHandle -= tokens;
            potentialPlayers.RemoveAt(0);
        }
        ...
        if (!OptionGroupSingleton<ChaosTokensOptions>.Instance.TokenHandingPriority)
        {
    ...
    public static void BeforeMurderEventHandler(BeforeMurderEvent @event)
    {
        ...
        if (target.HasModifier<TokenDefense>())
        {
            ...
            if (source.AmOwner)
            {
                ...
                Coroutines.Start(MiscUtils.CoFlash(MiraModuleColors.ChaosTokens));
