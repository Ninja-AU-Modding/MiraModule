...
public static class PlayerMaterialGradientUtils
{
    public static void Apply(int colorId, Renderer renderer)
    {
        ...
        var isGradient = MiraModuleGradientColors.TryGetSecondary(colorId, out var secondaryMain, out _);
        if (isGradient)
        {
    ...
    public static void Apply(Color color, Renderer renderer)
    {
        ...
        if (foundId >= 0 && MiraModuleGradientColors.TryGetSecondary(foundId, out var secondaryMain, out _))
        {
            var mat = renderer.material;
            var target = mat.HasProperty(ShaderID.Mask)
                ? MiraModuleGradientAssets.MaskedGradientMaterial.LoadAsset()
